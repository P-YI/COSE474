...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        H_out = int(1 + (H - H2)/stride)
        W_out = int(1 + (W - W2)/stride)
        out = torch.zeros((N, F, H_out, W_out))

        for h in range(H_out):
          for w in range(W_out):
            x_crop = x[torch.arange(N), :, h*stride:h*stride+H2, w*stride:w*stride+W2]
            x_crop_stretch = x_crop.reshape(N, f_size)
            w_stretch = self.W.reshape(F, f_size)
            sp_out = torch.matmul(x_crop_stretch, w_stretch.T) + self.b.reshape((1, F))
            out[torch.arange(N), :, h, w] = sp_out
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        W_out = int(1 + (W - pooling_W) / stride)
        H_out = int(1 + (H - pooling_H) / stride)
            ...
            for c in range(C):
                for w in range(W_out):
                    for h in range(H_out):
                        str_W = w*stride
                        str_H = h*stride
                        recep_f = x[b, c, str_W:str_W+pooling_W, str_H:str_H+pooling_H]
                        out[b,c,w,h] = torch.max(recep_f)
